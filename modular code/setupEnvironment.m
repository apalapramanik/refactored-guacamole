...
%     surfaces{end+1} = createWall([0 0 envHeight], [envWidth 0 envHeight], true); % Ceiling
...
% 
%     % Add back wall
%     surfaces{end+1} = createWall([x_start, y_start, 0], [x_start + width, y_start, height], true);
% 
%     % Add front wall
%     surfaces{end+1} = createWall([x_start, y_start + depth, 0], [x_start + width, y_start + depth, height], true);
% 
%     % Add left wall
%     surfaces{end+1} = createWall([x_start, y_start, 0], [x_start, y_start + depth, height], true);
% 
%     % Add right wall
%     surfaces{end+1} = createWall([x_start + width, y_start, 0], [x_start + width, y_start + depth, height], true);
% end
...
%     ];
%     if show
%         fill3(vertices(:, 1), vertices(:, 2), vertices(:, 3), [0.9, 0.9, 0.9]); % Light gray wall
%     end
% end
% 
...
%     ];
%     if show
%         fill3(vertices(:, 1), vertices(:, 2), vertices(:, 3), [0.8, 0.8, 0.8]); % Slightly darker gray floor
%     end
% end
...
function plotSurfaces(surfaces)
    ...
    for i = 1:length(surfaces)
        fill3(surfaces{i}(:, 1), surfaces{i}(:, 2), surfaces{i}(:, 3), [0.9, 0.9, 0.9]); % Light gray walls
    end
end
