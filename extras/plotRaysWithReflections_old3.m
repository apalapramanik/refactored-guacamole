        ...
        for phi = phiValues
            ...
            for reflection = 0:maxReflections
                ...
                if ~isempty(closestIntersection)
                    ...
                    plotRaySegment(currentOrigin, closestIntersection, pr, freq, stepSize, totalDistance);

                    % Apply reflection coefficient for the next segment
                    currentPower = pr - 10 * log10(reflection_coefficient);
...
function plotRaySegment(startPoint, endPoint, pr, freq, stepSize, totalDistance)
...
function color = powerToColor(pr)
    ...
    colormapData = parula(256); % Generate a colormap with 256 colors
